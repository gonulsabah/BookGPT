...
with st.sidebar:
    ...
    # Semantik Skorlama Alanı Eklendi
    st.markdown("---")
    ...
    # Publish Date (Yayın Yılı) Filtresi Eklendi
    st.markdown("---")
...
# Nasıl Çalışır? Kartları (3 Sütun)
...
# ==========================================
# ARAMA VE ÖNERİ BÖLÜMÜ (TABS KULLANIMI)
# ==========================================
# İki farklı arama türünü temiz göstermek için sekmeler (Tabs) ekledik
tab1, tab2 = st.tabs(["✨ Yapay Zeka Önerisi", "🔍 Doğrudan Metin Arama"])
...
with tab2:
    # YENİ EKLENEN METİN ARAMA BUTONU VE ALANI
    st.markdown("<h4 style='color: #E5A93C; margin-bottom: -15px; font-size: 22px;'>🔎 Kitap içeriklerinde veya isimlerinde spesifik kelime ara:</h4>", unsafe_allow_html=True)
    text_search_query = st.text_input(
        "", 
        placeholder="Örn: Yüzüklerin Efendisi, simülasyon teorisi, İstanbul...",
        key="text_search_query"
    )

    col_sbtn1, col_sbtn2, col_sbtn3 = st.columns([1, 1.5, 1])
    with col_sbtn2:
        search_button = st.button("🔍 Kitaplarda Ara", use_container_width=True)

    # Metin Arama İstek Mantığı
    if search_button:
        if text_search_query:
            try:
                with st.spinner("🔍 Kelime bazlı indeksler taranıyor..."):
                    # Backend'deki doğrudan arama endpoint'iniz hangisiyse (örn: /search) ona istek atıyor
                    response = requests.get(
                ...
                if response.status_code == 200:
                    ...
                    st.markdown(f"<h3 style='color: #E5A93C;'>🔎 Arama Sonuçları ({len(search_data)} sonuç):</h3>", unsafe_allow_html=True)
                    
                    if isinstance(search_data, list) and len(search_data) > 0:
                        for idx, book in enumerate(search_data, 1):
                            title = book.get("title", book.get("book_name", "Bilinmeyen Kitap"))
                            author = book.get("author", book.get("authors", "Bilinmeyen Yazar"))
                            description = book.get("description", book.get("summary", "Açıklama bulunmuyor."))
                            
                            st.markdown(f"""
                            <div style="background-color: #2E343A; padding: 22px; border-radius: 12px; border-left: 6px solid #4A90E2; margin-bottom: 18px; box-shadow: 0px 4px 15px rgba(0,0,0,0.2);">
                                <h4 style="margin: 0; color: #FFFFFF; font-size: 19px; font-weight: 700;">{idx}. {title}</h4>
                                <p style="margin: 6px 0; color: #4A90E2; font-weight: bold; font-size: 14px;">✍️ Yazar: {author}</p>
                                <hr style="border: 0; border-top: 1px solid #4E4741; margin: 12px 0;">
                                <p style="margin: 5px 0 0 0; color: #E1D9D1; font-size: 15px; line-height: 1.6;">{description}</p>
                            </div>
                            """, unsafe_allow_html=True)
                    else:
                        st.info("Aradığınız kelimeye uygun bir içerik bulunamadı.")
                else:
                    st.error(f"Backend Hatası: {response.status_code}")
            except Exception as e:
                st.error(f"💥 Bağlantı Hatası: {e}")
        else:
            st.warning("Lütfen aramak istediğiniz metni yazın!")